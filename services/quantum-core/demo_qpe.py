...
phi_est = k / 2^t, где k — измеренная битовая строка, переведённая в число, t=3 — число counting-кубит (отсюда разрешение 1/2³ = 1/8,
 то есть QPE в принципе не может различить значения φ, отличающиеся меньше чем на 0.125).
...
Именно поэтому вероятность распределилась в основном между ними (573+279=852 из 1024, то есть 83%), а не сконцентрировалась в одной точке. Чем ближе истинное φ к узлу сетки k/8, тем острее пик; чем дальше — тем сильнее размазывание. Здесь φ=0.3 ближе к 0.25, поэтому 010 доминирует над 011.
